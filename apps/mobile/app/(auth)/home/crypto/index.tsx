            ...
            {coins.map((coin: Coins) => (
              <Link href={`/home/crypto/${coin.id}`} key={coin.id} asChild>
                <TouchableOpacity style={{ flexDirection: "row", gap: 14, alignItems: "center" }}>
                  <Image source={{ uri: coinInfo?.[coin.id]?.["logo"] }} style={{ width: 40, height: 40 }} />
                  ...
                  <View style={{ gap: 6, alignItems: "flex-end" }}>
                    <Text className="font-SpaceGroteskBold text-darkBlue">{coin.quote.USD.price.toFixed(2)} $</Text>
                    <View className="gap-1 flex-row items-center">
                      ...
                      />
                      <Text style={{ fontFamily: "SpaceGrotesk-Regular", color: coin.quote.USD.percent_change_1h > 0 ? "green" : "red" }}>
