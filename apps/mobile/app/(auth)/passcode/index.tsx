...
export default function PassCodeScreen() {
  ...
  return (
    <View className="flex flex-1 justify-center items-center bg-white">
      ...
      {showLoadingModal && <LoadingModal modalVisible={showLoadingModal} color={Colors.yellow} />}

      <DialPad onPress={handlePress} />
      {mode === "confirm" && isPinMatch !== null && (
        <Text className="text-red-500 top-7 font-SpaceGroteskMedium">{isPinMatch ? "" : "Passcode does not match"}</Text>
      )}
    </View>
  );
}
