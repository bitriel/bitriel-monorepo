...
export default function ReceiveAddressScreen() {
  ...
  const name = (params.name as string) || 'SOL address';
  const address =
  ...
  const handleShare = async () => {
    ...
    try {
      await Share.share({
        message: `My ${symbol} address: ${address}`,
      });
    } catch (error) {
  ...
  const handleRequest = () => {
    ...
    router.push({
      pathname: '/(routes)/receive/request' as any,
    ...
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      <View className="flex-1 items-center justify-center px-5">
        ...
        <Text className="mb-8 text-center text-base opacity-80">
          Only send{' '}
          {symbol === 'DOMAIN' ? 'to this domain' : `Solana network tokens to this\naddress`}
        </Text>
        ...
        >
          ...
          {/* QR Code Placeholder */}
          <View
            className="size-52 items-center justify-center rounded-2xl"
        ...
        <View className="w-full gap-3">
          ...
          >
            ...
            <Text className="text-base font-medium">Request</Text>
          </Pressable>
          <Pressable
            onPress={handleShare}
            className="flex-row items-center justify-center gap-2 rounded-full py-4 active:opacity-90"
