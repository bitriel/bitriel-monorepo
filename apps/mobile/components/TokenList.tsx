    ...
    <View className="flex-1">
      ...
      <View className="space-y-2">
        {allTokens.map((token, index) => (
          <TouchableOpacity key={index} className="flex-row items-center bg-white p-4 rounded-xl border border-gray-100">
            {/* Token Logo */}
            {token.token.logoURI ? (
              <Image source={{ uri: token.token.logoURI }} className="w-10 h-10 rounded-full" contentFit="contain" />
            ) : (
            ...
            <View className="items-end">
              <Text className="text-base font-semibold text-gray-900">{formatBalance(token.balance, token.token.decimals)}</Text>
