...
const TokenListBottomSheet = forwardRef<Ref, BottomSheetProps>((props, ref) => {
  ...
  const renderBackdrop = useCallback(
    (backdropProps: any) => <BottomSheetBackdrop {...backdropProps} appearsOnIndex={0} disappearsOnIndex={-1} opacity={0.5} />,
    []
  );
  ...
  const filteredTokens = useMemo(() => {
    return props.tokens.filter((token) => {
      ...
      return (
        ...
        (token.token.symbol.toLowerCase().includes(searchLower) || token.token.name.toLowerCase().includes(searchLower))
      );
    });
        ...
        {filteredTokens.length > 0 ? (
          Object.entries(groupedTokens).map(([network, tokens]) => (
            <View key={network}>
              <View className="flex-row items-center px-4 py-3">
                ...
                <Text className="text-gray-500 text-base font-SpaceGroteskMedium uppercase">{network}</Text>
          ...
          <View className="flex-1 items-center justify-center px-6 py-12">
            <View className="w-16 h-16 mb-4 items-center justify-center">
              <MaterialIcons name={searchQuery.length > 0 ? "search" : "public"} size={48} color={Colors.darkBlue} />
            </View>
            <Text className="text-lg text-gray-400 font-SpaceGroteskMedium text-center mb-2">
              {searchQuery.length > 0 ? "No network or tokens with entered name were found." : "You don't have tokens to send."}
            </Text>
            <Text className="text-base text-gray-400 font-SpaceGroteskRegular text-center mb-6">
              {searchQuery.length > 0 ? "Try searching with a different name" : "Buy or Receive tokens to your account."}
            </Text>
            {/* {searchQuery.length === 0 && (
              <TouchableOpacity
                className="py-2 px-4"
