      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} className="flex-row">
        {SUPPORTED_NETWORKS.map((network) => (
          <TouchableOpacity
            ...
            disabled={isLoading}>
            {network.logo && <Image source={{ uri: network.logo }} style={{ width: 20, height: 20, marginRight: 8 }} contentFit="contain" />}
            <Text className={`${currentNetwork?.chainId === network.chainId ? "text-white" : "text-gray-800"} font-medium`}>{network.name}</Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
    </View>
