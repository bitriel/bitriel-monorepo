...
import { View, Text, StyleSheet, SafeAreaView, TouchableOpacity, Animated, Image, TextInput, Modal } from "react-native";
  ...
  return (
    <>
      <View className="my-5 flex-row justify-center items-center">
        <Image source={{ uri: logo }} className="w-10 h-10 rounded-full bg-white" style={{ objectFit: "contain" }} />
    ...
    <>
      <ViewShot ref={qrRef}>
        <View className="w-2/3 self-center items-center mb-5 bg-white rounded-3xl shadow-md p-5">
          ...
          <QRCode value={`${address}?amount=${amount}`} logo={logoFromFile} logoBackgroundColor="white" size={200} />
        ...
        <View className="flex-1 justify-center items-center bg-black/50">
          <View className="bg-white rounded-2xl w-[80%] p-4">
            <View className="gap-3">
              ...
              <TouchableOpacity className="bg-yellow p-3 rounded-lg" onPress={() => handleConfirmAmount(amount)}>
...
export const ReceiveScreen = ({ address, networkName, symbol, logo, tokenName }: ReceiveScreenProps) => {
  return (
    <SafeAreaView className="flex-1">
      <View className="py-5 mx-3">
        <HeaderWarnMsg networkName={networkName} />
        {logo && tokenName && <TokenDisplay logo={logo} name={tokenName} />}

        <QrCode address={address} symbol={symbol} />
      </View>
    </SafeAreaView>
  );
};
